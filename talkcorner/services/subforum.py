...
async def get_subforums(
    *, offset: int, limit: int, holder: DatabaseHolder
) -> List[Subforum]:
    return await holder.subforum.read_all(offset=offset, limit=limit)


async def get_subforum(*, subforum_id: int, holder: DatabaseHolder) -> Subforum:
    return await holder.subforum.read_by_id(subforum_id=subforum_id)


async def create_subforum(
    *, subforum_create: SubforumCreate, holder: DatabaseHolder, user: User
) -> Subforum:
    parent_forum = await holder.forum.read_by_id(
        forum_id=subforum_create.parent_forum_id
    )
...
async def patch_subforum(
    *,
    ...
    if subforum_patch.parent_forum_id:
        parent_forum = await holder.forum.read_by_id(
            forum_id=subforum_patch.parent_forum_id
        )
    ...
    if subforum_patch.child_forum_id:
        child_forum = await holder.forum.read_by_id(
            forum_id=subforum_patch.child_forum_id
        )
    ...
    try:
        patched_subforum = await holder.subforum.patch(
            subforum_id=subforum_id, creator_id=user.id, subforum_patch=subforum_patch
        )
        await holder.commit()
    except BaseAppException as e:
        await holder.rollback()
...
async def delete_subforum(
    *, subforum_id: int, holder: DatabaseHolder, user: User
) -> Subforum:
    deleted_subforum = await holder.subforum.delete(
        subforum_id=subforum_id, creator_id=user.id
    )
    await holder.commit()
