...
async def get_topic_message(
    *, topic_message_id: uuid.UUID, holder: DatabaseHolder
) -> TopicMessage:
    return await holder.topic_message.read_by_id(topic_message_id=topic_message_id)


async def create_topic_message(
    *, topic_message_create: TopicMessageCreate, holder: DatabaseHolder, user: User
) -> TopicMessage:
    try:
...
async def delete_topic_message(
    *, topic_message_id: uuid.UUID, holder: DatabaseHolder, user: User
) -> TopicMessage:
    deleted_topic_message = await holder.topic_message.delete(
        topic_message_id=topic_message_id, creator_id=user.id
    )
    await holder.commit()
