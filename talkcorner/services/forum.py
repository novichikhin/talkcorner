...
async def get_forums(*, offset: int, limit: int, holder: DatabaseHolder) -> List[Forum]:
    return await holder.forum.read_all(offset=offset, limit=limit)


async def get_forum(*, forum_id: int, holder: DatabaseHolder) -> Forum:
    return await holder.forum.read_by_id(forum_id=forum_id)


async def create_forum(
    *, forum_create: ForumCreate, holder: DatabaseHolder, user: User
) -> Forum:
...
async def patch_forum(
    *, forum_id: int, forum_patch: ForumPatch, holder: DatabaseHolder, user: User
) -> Forum:
    updated_forum = await holder.forum.patch(
        forum_id=forum_id, creator_id=user.id, forum_patch=forum_patch
    )
    await holder.commit()
...
async def delete_forum(*, forum_id: int, holder: DatabaseHolder, user: User) -> Forum:
