...
def nats_build_connection_uri(*, host: str, port: int, user: str, password: str) -> str:
...
async def js_create_or_update_stream(js: JetStreamContext, stream_name: str) -> None:
    subjects = [f"{stream_name}.broadcast.email.*"]
    try:
        await js.update_stream(
            config=api.StreamConfig(name=stream_name, subjects=subjects)
        )
